...
/* CTR_DRBG options */
/*#define MBEDTLS_CTR_DRBG_ENTROPY_LEN               48 < Amount of entropy used per seed by default
 * (48 with SHA-512, 32 with SHA-256)
 */
/*#define MBEDTLS_CTR_DRBG_RESEED_INTERVAL        10000 < Interval before reseed is performed by
 * default
 */
 ...
 * sources
 */
/*#define MBEDTLS_ENTROPY_MIN_HARDWARE               32 < Default minimum number of bytes required
 * for the hardware entropy source mbedtls_hardware_poll() before entropy is released
 */
...
/*#define MBEDTLS_PLATFORM_STD_NV_SEED_READ   mbedtls_platform_std_nv_seed_read < Default
 * nv_seed_read function to use, can be undefined
 */
/*#define MBEDTLS_PLATFORM_STD_NV_SEED_WRITE  mbedtls_platform_std_nv_seed_write < Default
 * nv_seed_write function to use, can be undefined
 */
/*#define MBEDTLS_PLATFORM_STD_NV_SEED_FILE  "seedfile" < Seed file to read/write with default
 * implementation
 */
 ...
 * undefined. See MBEDTLS_PLATFORM_STD_CALLOC for requirements.
 */
 ...
 * MBEDTLS_HAVE_TIME must be enabled
 */
/*#define MBEDTLS_PLATFORM_TIME_TYPE_MACRO       time_t < Default time macro to use, can be
 * undefined. MBEDTLS_HAVE_TIME must be enabled
 */
/*#define MBEDTLS_PLATFORM_FPRINTF_MACRO      fprintf < Default fprintf macro to use, can be
 * undefined
 */
 ...
 * undefined
 */
/*#define MBEDTLS_PLATFORM_VSNPRINTF_MACRO    vsnprintf < Default vsnprintf macro to use, can be
 * undefined
 */
/*#define MBEDTLS_PLATFORM_NV_SEED_READ_MACRO   mbedtls_platform_std_nv_seed_read < Default
 * nv_seed_read function to use, can be undefined
 */
 ...
 * MBEDTLS_PLATFORM_MS_TIME_TYPE_MACRO   int64_t < Default milliseconds time macro to use, can be
 * undefined. MBEDTLS_HAVE_TIME must be enabled. It must be signed, and at least 64 bits. If it is
 * changed from the default, MBEDTLS_PRINTF_MS_TIME must be updated to match.
 */
/*#define MBEDTLS_PRINTF_MS_TIME    PRId64 < Default fmt for printf. That's avoid compiler warning
 * if mbedtls_ms_time_t is redefined
 */
...
/* RSA OPTIONS */
/*#define MBEDTLS_RSA_GEN_KEY_MIN_BITS            1024 < Minimum RSA key size that can be generated
 * in bits (Minimum possible value is 128 bits)
 */
