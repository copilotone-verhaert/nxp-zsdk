...
This section describes building and usages of coexistence example on Zephyr OS for target modules. The coexistence example combines Bluetooth and Wi-Fi commands into one single example and supports concurrent Wi-Fi and Bluetooth operations.

## Build Coexistence Example

List below Wi-Fi + BT coexistence examples supported by each wireless chip:
...
> **Note:** The BT example path is to specify Bluetooth example folder path which as parameters used in coexistence example build commands

Below shows how to use west command build target coexistence examples:

- Build coex example with embedded Wi-Fi supplicant
```bash
...
- Build coex example with host based supplicant
...
Flash coexistence example binary and reboot system, give “help” to show all supported commands, for all Wi-Fi related commands encapsulated under “wifi” menu and “nxp_wifi” menu, for all Bluetooth shell command encapsulated under “bt” menu:

![示例图片](../images/coex_1.png)

Take example of below steps to establish the BLE and Wi-Fi connection:
...
```
- BLE connection with remote device
```bash
#bt connect <remote BT address> <address type>
```
- Wi-Fi connection with external AP(configure channel in 2.4Ghz) in WPA2 personal mode
```bash
#wifi connect -s <ssid> -p <password> -k 1
```

- Check the data traffic on Wi-Fi interface using ping
```bash
#net ping <IP address>
```

- Check the status of Wi-Fi connection
```bash
#wifi status
```

- Check the status of BLE connection
```bash
#bt info
```
or
...
This chapter describes NXP coexistence proprietary commands supported in zephyr example.
...
|Ieee154Duration|Set 15.4 duration time <br> Enter value in Units(1Unit=1ms)|
|TotalDuration|Set total duty-cycle duration time <br> Enter value in Units(1Unit=1ms)|

> **Note:** Ieee154Duration should not equal to TotalDuration-Ieee154Duration and no more than TotalDuration
...
|Ieee154Duration|Set 15.4 duration time <br> Enter value in Units(1Unit=1ms)|
|TotalDuration|Set total duty-cycle duration time <br> Enter value in Units(1Unit=1ms)|
|Ieee154FarRangeDuration|Set 15.4 far range duration <br> Enter value in Units (1Unit = 1ms)|
> **Note:** Ieee154Duration and Ieee154FarRangeDuration should not equal to TotalDuration-Ieee154Duration, Ieee154Duration should no more than TotalDuration
...
|ExtWifiBtArb|Enable/disable Wi-Fi/BT/MWS arbitration <br> Enable Ext-WifiBtArb: 1 <br>Disbale Ext-WifiBtArb: 0 |
...
|SampTiming|Set sample timing of Priority bit <br> Sample timing range [20, 200] <br> Defalut value: 100 <br> Enter value in Units (1Unit = 1ms)|
|InfoSampTiming|Set sample timing of Tx/Rx info <br> Sample timing range [20, 200] <br> Default value: 100 <br> Enter value in Units (1Unit = 1ms)|
|TrafficPrio|Enable external traffic Tx/Rx Priority: 1 <br> Disable external traffic Tx/Rx Priority: 0|
|CoexHwIntWic|Enable WCI-2 interface: 1 <br> Disable WCI-2 interface: 0|
